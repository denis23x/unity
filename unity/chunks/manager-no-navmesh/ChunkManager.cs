// Assets/Editor/ChunkManagerNoNavmesh/ChunkManager.cs
//
// Simplified variant of ChunkManager with the Navmesh tab removed. Use this
// when the project doesn't ship A* Pathfinding Pro (NavmeshPrefab /
// RecastNavmeshModifier types) — keeping the full ChunkManager would fail
...
//   * Apply Tags / Apply Layers — recursively walks every open chunk scene
//     and assigns a project-defined Tag or Layer to GameObjects whose name
//     starts with one of the user-defined key prefixes. Tag/Layer values
//     are picked from the project's existing entries via TagField /
//     LayerField, so this step never creates new tags or layers.
//
...
//
// Menu: Tools → Chunks → Chunk Manager (No Navmesh)
...
{
    public partial class ChunkManagerNoNavmesh : EditorWindow
    {
        const string PK = "ChunkManagerNoNavmesh.";
        ...
        [MenuItem("Tools/Chunks/Chunk Manager (No Navmesh)")]
        static void Open() => GetWindow<ChunkManagerNoNavmesh>("Chunk Manager (No Navmesh)");
        ...
        void BindFields(VisualElement root)
        {
            ...
            // Chunk Tags and Layers — the configs lists are the source of truth;
            // empty lists short-circuit the Apply button.
        ...
        // Tag / Layer config rows are built imperatively because the row count
        // is dynamic and each row carries Add/Remove handlers that capture the
        // config's list index. Existing rows are replaced wholesale because the
        // index closures are cheaper to recreate than to retarget.
        // Each card has a foldout header (showing the key prefix), a Remove
