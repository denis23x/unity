// Assets/Editor/ChunkManagerNoNavmesh/ChunkManager.Import.cs
//
...
namespace ProjectName.EditorTools
{
    public partial class ChunkManagerNoNavmesh
    {
        ...
        void ImportChunks()
        {
            ...
            if (entries.Count == 0)
            {
                EditorUtility.DisplayDialog("Chunk Manager (No Navmesh)", "Found 0 XX_YY.fbx files in source folder.", "OK");
                return;
            }
            ...
            Debug.Log($"[ChunkManagerNoNavmesh] {entries.Count} FBX files; " +
                      $"grid a:{minA}..{maxA} ({countA}), b:{minB}..{maxB} ({countB}); cell={chunkSize}m");

            if (!EditorSceneManager.SaveCurrentModifiedScenesIfUserWantsTo())
            {
                Debug.LogWarning("[ChunkManagerNoNavmesh] Aborted by user (unsaved scenes).");
                return;
            }
            ...
            Debug.Log($"[ChunkManagerNoNavmesh] DONE. {written}/{entries.Count} scenes written to {destFolder}.");
        ...
        bool ImportOne(Entry e, int minA, int minB, int countA, int countB)
        {
            ...
            if (fbx == null)
            {
                Debug.LogError($"[ChunkManagerNoNavmesh] Could not load FBX asset: {e.assetPath}");
            ...
            if (!ok) Debug.LogError($"[ChunkManagerNoNavmesh] Failed to save: {scenePath}");
            else     Debug.Log($"[ChunkManagerNoNavmesh] {baseName}  →  root @ ({rootPos.x:F1}, {rootPos.y:F1}, {rootPos.z:F1})  [col={col}, row={row}]");
        ...
        void BakeChunkToIdentity(GameObject root, GameObject inst, string baseName)
        {
            ...
            for (int i = 0; i < meshFilters.Length; i++)
            {
                ...
                {
                    Debug.LogWarning($"[ChunkManagerNoNavmesh] {baseName}: source mesh '{source.name}' is not readable; skipping bake for this filter.");
                    continue;
                }
